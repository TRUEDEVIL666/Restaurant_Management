...
class _ComboSelectionScreenState extends State<ComboSelectionScreen> {
  ...
  Future<void> _loadSavedSelections() async {
    ...
    final tableKey = 'table_${widget.tableNumber}';
    ...
    setState(() {
      ...
      for (var price in ticketPrices) {
        selectedTicketCounts[price] = prefs.getInt('${tableKey}_ticket_$price') ?? 0;
      }
    });
  }
  ...
  Future<void> _saveSelections() async {
    ...
    await prefs.setInt('${tableKey}_openTime', tableOpenTime.millisecondsSinceEpoch);
  ...
  final tableNumber = widget.tableNumber.toString();
  final firestore = FirebaseFirestore.instance;

  // Tạo billId dạng "bill_tableNumber_timestamp"
  String billId = 'bill_${widget.tableNumber}_${DateTime.now().millisecondsSinceEpoch}';
...
void _saveAndContinue() async {
  ...
  Navigator.pushReplacement(
    ...
    MaterialPageRoute(
      builder: (context) => TableDetailScreen(
        tableNumber: widget.tableNumber,
        onCloseTable: widget.onCloseTable,
      ),
    ),
  );
}
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Bàn ${widget.tableNumber} - Chọn Combo & Giá Vé'),
      ),
      body: Padding(
  ...
  Widget _buildDrinkComboSelection() {
    return Column(
      children: drinkCombos.map((combo) {
        return Row(
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Text('${selectedCombos[combo]}', style: TextStyle(fontSize: 16)),
                IconButton(
  ...
  Widget _buildTicketPriceSelection() {
    return Column(
      children: ticketPrices.map((price) {
        return Row(
          mainAxisAlignment: MainAxisAlignment.spaceBetween,
          children: [
                  ...
                  onPressed: () {
                    setState(() {
                      if (selectedTicketCounts[price]! > 0) {
                        selectedTicketCounts[price] = selectedTicketCounts[price]! - 1;
                      }
                    });
                  },
                ),
                Text('${selectedTicketCounts[price]}', style: TextStyle(fontSize: 16)),
                IconButton(
                  icon: Icon(Icons.add_circle_outline),
                  onPressed: () {
                    setState(() {
